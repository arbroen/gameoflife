...
from game_of_life.algorithm import still_alive, world_generator
...
class TestAlgorithm:
    ...
    def test_game_generator(self, small_world, medium_world):
        ...
        """
        all_worlds = [
            world for world in world_generator(world=small_world, generations=0)
        ]

        assert len(all_worlds) == 7

        all_medium_worlds = [
            world for world in world_generator(world=medium_world, generations=0)
        ]
        ...
        food_for_though = [
            world for world in world_generator(world=overpopulated_world, generations=0)
        ]
